...
| V2.3 | 2026-09-09 | **产品重定位**：由「四步创作工具（提示词→歌词→曲谱→MIDI）」改为「MiniMax 提示词输出专用工具（三步：提示词→歌词→导出）」。第 2 章功能需求以本文档当前正文为准的历史 M3 曲谱 / M4 MIDI 模块移出主流程（代码保留于 legacy 目录作为 P1 兜底）。新增：MiniMax 格式校验模块（确定性算法）、导出提示词包、结构化元标签生成。外部依赖说明：大模型生成经由云端 API（DeepSeek V4 系列 / 通义千问），成品音频由 MiniMax 音乐网页版生成，本系统不本地部署大模型。 | 古韵AI项目组 |

> **V2.3 阅读指引**：本文档第 2 章以下 M3（曲谱生成）、M4（MIDI 导出）相关章节描述的是 P1 兜底管线的需求基线，主流程已不经过；判断当前实际行为以 `backend/app/routers/minimax.py`（格式规格单一事实来源）与 `文档/重构变更说明.md` 为准。
